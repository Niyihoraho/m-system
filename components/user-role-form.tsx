...
export function UserRoleForm({ userId, userName, onSuccess, onCancel }: UserRoleFormProps) {
  ...
  const fetchAlumniGroups = async (regionId: number) => {
    try {
      const response = await fetch(`/api/alumni-groups?regionId=${regionId}`);
      const data = await response.json();
              ...
              {formData.regionId && (
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <SelectTrigger className="h-11">
                        <SelectValue placeholder="Select alumni group (optional)" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="">No alumni group</SelectItem>
